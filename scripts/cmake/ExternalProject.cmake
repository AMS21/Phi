...
function(mock_target)
  ...
  foreach(file ${sourceFilesRaw})
    get_filename_component(path ${file} REALPATH BASE_DIR ${mock_BASE_DIR})
  ...
  if(systemIncludeDirs)
    set_target_properties(${mock_NAME} PROPERTIES INTERFACE_SYSTEM_INCLUDE_DIRECTORIES
                                                  "${systemIncludeDirs}")
  endif()
...
function(phi_add_external_project)
  ...
  foreach(target ${ext_TARGETS})
    ...
    # If the target is a static library, we create a seperate static library with PIC enabled so it
    # can be used from shared libraries
    if(${target_type} STREQUAL "STATIC_LIBRARY" AND NOT PHI_COMPILER_MSVC)
      set(mock_name ${target}_dyn)

      mock_target(NAME ${mock_name} TARGET ${target} BASE_DIR
                  "${CMAKE_CURRENT_SOURCE_DIR}/${ext_PROJECT}")
