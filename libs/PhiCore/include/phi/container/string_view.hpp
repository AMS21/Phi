...
public:
    ...
    constexpr basic_string_view(nullptr_t) noexcept
        : m_Data{nullptr}
        , m_Length{0u}
    {}
...
public:
    ...
    template <size_t Size>
    constexpr PHI_ATTRIBUTE_NONNULL not_null_basic_string_view(CharT (&array)[Size]) noexcept
        : m_Data{array}
        , m_Length{Size}
    {
    ...
    template <size_t Size>
    constexpr not_null_basic_string_view(const array<CharT, Size>& array) noexcept
        : m_Data{array.data()}
        , m_Length{array.size()}
    {
        PHI_ASSERT(m_Data != nullptr);
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_RETURNS_NONNULL constexpr iterator begin() const noexcept
    {
        return m_Data;
    }

    PHI_NODISCARD PHI_ATTRIBUTE_RETURNS_NONNULL constexpr iterator end() const noexcept
    {
        return m_Data + m_Length.unsafe();
    }

    PHI_NODISCARD PHI_ATTRIBUTE_RETURNS_NONNULL constexpr const_iterator cbegin() const noexcept
    {
        return m_Data;
    }

    PHI_NODISCARD PHI_ATTRIBUTE_RETURNS_NONNULL constexpr const_iterator cend() const noexcept
    {
        return m_Data + m_Length.unsafe();
    }
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_RETURNS_NONNULL constexpr const_pointer data() const noexcept
    {
        return m_Data;
    }
    ...
    PHI_NODISCARD constexpr i32 compare(not_null_basic_string_view other) const noexcept
    {
#if PHI_HAS_FEATURE_EXTENDED_CONSTEXPR()
        {
            const i32 result = TraitsT::compare(data(), other.data(),
    ...
    PHI_NODISCARD constexpr i32 compare(size_type pos1, size_type count1,
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr i32 compare(const CharT* string) const noexcept
    {
        return compare(basic_string_view(string));
    }

    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr i32 compare(size_type pos, size_type count,
                                                              const CharT* string) const noexcept
    {
        return substring_view(pos, count).compare(basic_string_view(string));
    }

    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr i32 compare(size_type pos, size_type count1,
                                                              const CharT* string,
                                                              size_type    count2) const noexcept
    {
        return substring_view(pos, count1).compare(basic_string_view(string, count2));
    ...
    PHI_NODISCARD constexpr boolean starts_with(not_null_basic_string_view view) const noexcept
    {
        return length() >= view.length() && compare(0u, view.length(), view) == 0;
    }
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr boolean starts_with(
            const CharT* string) const noexcept
    {
        return starts_with(basic_string_view(string));
    }

    PHI_NODISCARD constexpr boolean ends_with(not_null_basic_string_view view) const noexcept
    {
        return length() >= view.length() && compare(length() - view.length(), npos, view) == 0;
    }
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr boolean ends_with(
            const CharT* string) const noexcept
    {
        return ends_with(basic_string_view(string));
    }
    ...
    PHI_NODISCARD constexpr boolean contains(not_null_basic_string_view view) const noexcept
    {
        return find(view) != npos;
    }
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr boolean contains(
            const CharT* string) const noexcept
    {
        return find(string) != npos;
    }
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr size_type find(const CharT* string,
                                                                 size_type pos = 0u) const noexcept
    {
        return find(basic_string_view(string), pos);
    }

    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr size_type find(const CharT* string, size_type pos,
                                                                 size_type count) const noexcept
    {
        return find(basic_string_view(string, count), pos);
    }
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr size_type rfind(
            const CharT* string, size_type pos = npos) const noexcept
    {
        return rfind(basic_string_view(string), pos);
    }

    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr size_type rfind(const CharT* string,
                                                                  size_type    pos,
                                                                  size_type    count) const noexcept
    {
        return rfind(basic_string_view(string, count), pos);
    ...
    PHI_NODISCARD constexpr size_type find_first_of(not_null_basic_string_view view,
                                                    size_type pos = 0u) const noexcept
    {
        return pos >= length() ? npos :
                                 to_pos(std::find_first_of(cbegin() + pos, cend(), view.cbegin(),
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr size_type find_first_of(
            const CharT* string, size_type pos = 0u) const noexcept
    {
        return find_first_of(basic_string_view(string), pos);
    }

    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr size_type find_first_of(
            const CharT* string, size_type pos, size_type count) const noexcept
    {
        return find_first_of(basic_string_view(string, count), pos);
    }
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr size_type find_last_of(
            const CharT* string, size_type pos = npos) const noexcept
    {
        return find_last_of(basic_string_view(string), pos);
    }
    ...
    PHI_NODISCARD constexpr size_type find_first_not_of(not_null_basic_string_view view,
                                                        size_type pos = 0u) const noexcept
    {
        return pos >= length() ? npos :
                                 to_pos(std::find_if(cbegin() + pos, cend(), not_in_view(view)));
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr size_type find_first_not_of(
            const CharT* string, size_type pos = 0u) const noexcept
    {
        return find_first_not_of(basic_string_view(string), pos);
    }
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr size_type find_last_not_of(
            const CharT* string, size_type pos = npos) const noexcept
    {
        return find_last_not_of(basic_string_view(string), pos);
    }
...
private:
    struct not_in_view
    {
        ...
        constexpr explicit not_in_view(not_null_basic_string_view other) noexcept
            : view(other)
        {}
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr size_type to_pos(
    ...
    PHI_NODISCARD PHI_ATTRIBUTE_NONNULL constexpr size_type to_pos(
            const_reverse_iterator position) const noexcept
    {
        return position == crend() ? npos : size_type(crend() - position - 1);
    }
...
template <typename CharT, typename TraitsT>
constexpr boolean operator==(not_null_basic_string_view<CharT, TraitsT> lhs,
