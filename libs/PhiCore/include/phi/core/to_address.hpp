...
namespace detail
{
    ...
    constexpr TypeT* to_address_impl(TypeT* pointer) noexcept
    {
    ...
    constexpr decay_t<decltype(to_address_impl_helper<PointerT>::call(declval<const PointerT&>()))>
    ...
    struct to_address_impl_helper
    {
        constexpr static decltype(to_address_impl(declval<const PointerT&>().operator->())) call(
                const PointerT& pointer) noexcept
        {
    ...
    {
        constexpr static decltype(pointer_traits<PointerT>::to_address(declval<const PointerT&>()))
...
template <typename TypeT>
constexpr auto to_address(TypeT* pointer) noexcept
{
    return detail::to_address_impl(pointer);
}

template <typename PointerT>
constexpr auto to_address(const PointerT& pointer) noexcept
