...
{
    PHI_ASSERT(string != nullptr, "Passing nullptr to string_length is not allowed. Use "
...
template <typename CharT, typename TraitsT, typename AllocatorT>
PHI_NODISCARD PHI_ATTRIBUTE_PURE constexpr usize string_length(
...
template <typename CharT>
PHI_NODISCARD PHI_ATTRIBUTE_PURE PHI_EXTENDED_CONSTEXPR usize
safe_string_length(CharT* string) noexcept
{
    if (string == nullptr)
    {
        return 0u;
...
template <typename CharT, typename TraitsT, typename AllocatorT>
PHI_NODISCARD PHI_ATTRIBUTE_PURE constexpr usize safe_string_length(
        std::basic_string<CharT, TraitsT, AllocatorT> string) noexcept
{
    return string.length();
}
...
template <typename CharT = nullptr_t>
PHI_NODISCARD PHI_ATTRIBUTE_PURE constexpr usize safe_string_length(nullptr_t) noexcept
{
    return 0u;
}
...
template <typename CharT>
PHI_NODISCARD PHI_ATTRIBUTE_PURE PHI_EXTENDED_CONSTEXPR usize
safe_string_length(CharT* string, usize length) noexcept
{
    if (string == nullptr)
    {
        return 0u;
...
template <typename CharT = nullptr_t>
PHI_NODISCARD PHI_ATTRIBUTE_PURE constexpr usize safe_string_length(
        nullptr_t, PHI_UNUSED usize length) noexcept
{
    return 0u;
}
